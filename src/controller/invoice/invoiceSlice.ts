...
const invoiceSlice = createSlice({
    ...
    reducers: {
        ...
        changeItem: (state: InvoiceState, action: PayloadAction<{ index: number, att: string, value: any }>) => {
            ...
            if (["tax", "discount"].indexOf(action.payload.att) !== -1) {
                if (value === "" || value < 0) {
                    value = 0;
                }
            }
            state.items[action.payload.index][action.payload.att] = value;
        },
        ...
        setErrorMessages: (state: InvoiceState, action: PayloadAction<string[]>) => {
